...
- **THE TIMEFRAMES ARE NOT ALL REQUIRED TO AGREE. This line used to claim they were.**
  ...
  actually does (`generateSignalMTF`, `server/index.js:3481`; the plain
  `generateSignal` is at `:2096`. This said `:2814-2911` until 2026-09-02 — that
  ...
  **Only ONE branch anywhere lets `h1` touch the confidence maths** — the
  triple-alignment bonus at `server/index.js:3624` — plus a display copy in the
  payload at `server/index.js:4005`. This line claimed "`h1` appears exactly TWICE in
...
- **"STRONG UPTREND" is EMA STACKING, NOT CANDLE DIRECTION** (`index.js:1975`;
...
- **Gold's squeeze cohort is pinned to a LITERAL 70** (`GOLD_SQUEEZE_MODERATE_CONFIDENCE`,
  `server/index.js:3574`; said `:3486`, then `:3553`, then `:3650` — rotted three times, twice by
