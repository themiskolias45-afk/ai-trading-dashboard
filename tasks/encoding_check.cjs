...
let files;
try {
  files = fs.readdirSync(dir).filter(f => /\.(html|css|js)$/i.test(f)).sort();
} catch (err) {
  console.error("encoding_check: cannot read " + dir + " - " + err.message);
  process.exit(2);
}
if (!files.length) {
  console.error("encoding_check: no html/css/js files in " + dir);
  process.exit(2);
}

// Report the line so the fix is `git checkout <clean-sha> -- <file>`, not a hunt.
const firstLines = (text, re, limit) => {
  const hits = [];
...
for (const name of files) {
  ...
  let buf;
  try { buf = fs.readFileSync(full); } catch (err) {
    rows.push({ name, note: "unreadable - " + err.message, bad: 0 });
    continue;
  }
  const text = buf.toString("utf8");
  const moji = (text.match(MOJIBAKE) || []).length;
  const repl = (text.match(REPLACEMENT) || []).length;
  // A UTF-8 BOM is not corruption, but it breaks some strict parsers - worth naming.
  const bom = buf.length >= 3 && buf[0] === 0xef && buf[1] === 0xbb && buf[2] === 0xbf;
  const bad = moji + repl;
  if (bad > 0) damaged++;
  rows.push({ name, moji, repl, bom, bad, text, full });
}

if (!quiet) {
  console.log("ENCODING CHECK - " + dir);
  for (const r of rows) {
    ...
    console.log("  " + r.name.padEnd(22) +
      "mojibake=" + String(r.moji).padStart(4) +
      "  U+FFFD=" + String(r.repl).padStart(3) +
      "  " + verdict);
    if (r.bad > 0) {
      for (const hit of firstLines(r.text, MOJIBAKE, 3)) {
...
if (damaged > 0) {
  console.log("RESULT: " + damaged + " file(s) hold mis-decoded text.");
  console.log("FIX: find the last commit before the damage and restore that file only -");
  console.log("     git log --oneline -5 -- <file>   then   git checkout <sha> -- <file>");
  console.log("     Re-apply the intended edit with an editor, never a PowerShell read/write pair.");
  console.log("     If you must script it: [System.IO.File]::ReadAllText($p) reads UTF-8;");
  console.log("     Get-Content -Raw does NOT on PowerShell 5.1.");
  process.exit(1);
}

console.log("RESULT: all " + rows.length + " file(s) clean.");
process.exit(0);
