...
echo.
"%PY%" mt5_bridge.py %BRIDGE_ARGS% >> tasks\logs\bridge_log_B.txt 2>&1
REM ---------------------------------------------------------------------------
...
REM   Start-Process cmd -ArgumentList '/c','tasks\start_bridge_B.bat'
...
echo [%DATE% %TIME%] start_bridge_B exited with code %BRIDGE_EXIT% >> "tasks\logs\bridge_starts.txt"
echo.
echo  Bridge B stopped (exit code %BRIDGE_EXIT%). Closing in 30s.
