...
// #property version   "3.55" // v3.56 suppressed
#property version   "3.56"   // v3.56 ADDITIVE: config sentry scores against the POST-2026-09-04 profile
#define EA_VER_TAG "v3.56"   // v3.56 ADDITIVE: one place, so the log prefix cannot rot again
...
int OnInit()
{
   ...
   if(InpShowConfigSentry)
   {
      ...
      // Print("v3.51 ", g_cfgSentryMsg); // v3.56 suppressed - hardcoded, so a v3.55 binary logged "v3.51"
      Print(EA_VER_TAG, " ", g_cfgSentryMsg);   // v3.56 ADDITIVE
...
void ValidateConfigSentry()
{
   ...
   // v3.56 ADDITIVE: only checked while the trail is actually RUNNING. InpTrailStartRR is
   // inert when InpUseTrailingStop is false, so scoring it there reported a difference in a
   // number that does nothing -- the "TrailRR 1.5<>1.2" that sat on the dashboard for days.
   double expTrail = 0.0;
   ...
   // if(!InpUseTrailingStop) issues += "TRAIL OFF  "; // v3.56 suppressed - this filed the FIX as drift
   // v3.56 ADDITIVE: the validated profile changed on 2026-09-04. Measured on XAUUSD M15 over
   // 13 months of real ticks: trail ON -15.28 GBP / trail OFF +536.27, PF 1.00 -> 1.18,
   // maxDD 9.71% -> 6.77%. OFF is now the tested cell, so ON is the drift. While this line
   // read the other way the dashboard showed a red CONFIG DRIFT that could not clear while
   // the correct config was loaded, and "CONFIG: VALIDATED" could only mean the losing one.
   if(InpUseTrailingStop) issues += "TRAIL ON(tested:off)  ";

   // v3.56 ADDITIVE: STATE THE FACT, do not make a reader infer it from the absence of a
   // complaint. Every prior misreading came from inferring the trail state from wording that
   // meant the opposite. This token appears on EVERY sentry line, drift or not.
   string trailState = (InpUseTrailingStop ? "TRAIL:ON" : "TRAIL:OFF");

   if(issues == "")
   {
      g_cfgSentryOK = true;
      g_cfgSentryMsg = "CONFIG: VALIDATED  [" + _Symbol + (expTrail > 0.0 ? " trail " + DoubleToString(expTrail, 1) : "") + " profile]  " + trailState;
   }
   else
   {
      g_cfgSentryOK = false;
      g_cfgSentryMsg = "CONFIG DRIFT: " + issues + " " + trailState;
   }
}
