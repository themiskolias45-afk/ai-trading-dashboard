@echo off
REM Sweeps BOTH terminals on this box into tasks\all_trades_ledger.jsonl.
REM
REM Two runs, not one: mt5.initialize() with no path attaches to whichever terminal
...
REM missed run is caught by the next one. Hourly is for surface freshness, not safety.
REM
