...
const CLAIMS = [
  ...
  },
  {
    ...
    evidence: "MOMENTUM must sit under RSI 72 and TREND_FOLLOW under 68. On 2026-08-22 "
      ...
      + "answer. "
      + "*** SUPERSEDED 2026-09-05 — EVERY NUMBER ABOVE WAS MEASURED ON THE WRONG RULER. "
      + "All three cuts above ran at MTF_MAX_HOLD=40, because rsi_ceiling_walkforward.cjs "
      + "never set it and _replay_mtf.cjs defaults it to 40. That horizon scores an "
      + "unresolved trade as EXPIRED while the live system has no max hold at all, and on "
      + "this axis it does not merely add noise — it REVERSES the answer. Re-run at "
      + "MTF_MAX_HOLD=320 across the same three cuts: only TWO candidates beat the live "
      + "88/84 baseline's worst fold under all of them — NO CEILING and 72/68. On the "
      + "per-asset cost basis: no ceiling 482 closed +182.6R worst +0.156 5/5; live 88/84 "
      + "475 closed +174.6R worst +0.117 5/5; 72/68 364 closed +128.2R worst +0.228 5/5. "
      + "72/68 flips outright with the horizon — worst −0.135 at 1/5 on hold 40, worst "
      + "+0.189 at 5/5 on hold 320, from the single worst option to the best worst-fold "
      + "performer. AND THE CLAIM ABOVE THAT 56/52 IS 'clearly worse: −0.326 worst, 2/5, "
      + "−0.4R total' IS SIMPLY WRONG: re-run at hold 40 — the very horizon that produced "
      + "it — 56/52 measures +0.068 worst, 5/5, +30.2R. Nothing resembles the recorded "
      + "figures. 56/52 was also the strategy searcher's 'promotable' pick on 2026-09-05 "
      + "and it FAILS the per-asset cost cut at the honest horizon, so one cut would have "
      + "shipped it. The old text is kept rather than deleted so the correction is "
      + "auditable; read the numbers before this marker as historical, not current. "
      + "Reproduce: MTF_MAX_HOLD=320 node tasks/rsi_ceiling_walkforward.cjs, then again "
      + "with RSI_CEILING_FOLD_MODE=time and with RSI_CEILING_COST=perasset. ***",
  ...
  },
  {
    ...
    changesTheAnswer: "A walk-forward comparing the live win-rate boost against an "
      ...
      + "+2.440R while being -$170.32, so 'R-weight it' would have loosened the very "
      + "boost that raised the alarm. "
      + "THE CASH SIGNAL'S SIGN IS A CONFIG EDIT, NOT A MEASUREMENT. fixedLotSize went "
      + "0.01 -> 0.02 on 2026-08-24T14:26Z. MOMENTUM's two LOSERS filled 08-25 and 08-28, "
      + "at the new size; its three winners filled at the old one. Mean 1R on the winners "
      + "GBP 52.11, on the losers GBP 195.14 — the losers carried 3.75x the money per "
      + "unit of risk, for reasons that have nothing to do with MOMENTUM. Re-price every "
      + "fill at ONE lot size, entries and exits untouched: 0.01 gives +GBP 12.66, 0.02 "
      + "gives +GBP 22.38, against an actual -GBP 170.32, and netR is +2.438 in all three "
      + "cases. Re-priced at 0.01, cashRate moves 0.360 -> 0.516 and the cash boost goes "
      + "-1 -> 0. So MOMENTUM's negative cash carries NO information about MOMENTUM, and "
      + "neither does the cash variant's sign. "
      + "WHY R IS NOT AUTOMATICALLY RIGHT EITHER. All five closed losers closed EXACTLY "
      + "at their stop — every one is exactly -1.00R — while their dollar losses span "
      + "$6.64 to $449.72, 68x for identical risk. And on 8 of 9 fills the risk "
      + "computation never reached the order at all: lots were pinned by fixedLotSize or "
      + "the broker's volume_min. So R is the only unit in which these nine fills are "
      + "commensurable, but that is a property of the SIZING pipeline, not evidence that "
      + "R-weighting the boost is better. "
      + "MEASURED, and it moves the question: a Monte Carlo over 60k simulated setups per "
      + "cell shows the crossover is NOT in n, it is in cross-setup payoff dispersion. At "
      + "sigma_b = 0 win rate beats R-expectancy at every n from 5 to 200; past roughly "
      + "sigma_b = 0.20 R-expectancy already wins AT n=5. The docblock's low-variance "
      + "argument silently assumes sigma_b ~ 0, and with 4 setups and one holding n>1 "
      + "sigma_b IS NOT CURRENTLY ESTIMABLE. Dollar-expectancy measured WORST of the "
      + "three at every n, because it inherits payoff dispersion and adds sizing noise. "
      + "AND THE LIVE CURVE HAS A KNOWN OPTIMISM BIAS: by exact binomial enumeration a "
      + "setup with genuinely ZERO edge receives an expected boost of +1.875 at n=5, "
      + "because the shrink neutralises the negative side at exactly the sample where "
      + "LEARNING_MIN_TRADES first lets the function act. The literature (van Zwet & "
      + "Cator; Mudd et al.) shrinks the side you SELECT on; this shrinks the other one. "
      + "That is defensible ONLY as a stated trade — accept ~+1.9pt optimism at n=5 to "
      + "protect accumulation — not as an absent problem. "
      + "BOTH FIXES ARE UNMEASURED ANYWAY: fixedLotSize went to 0 on 2026-09-02 and the "
      + "pointValue double-division fix landed 2026-09-05, while the last bridge fill "
      + "opened 2026-08-31. ZERO fills after either. The next bridge fill is the first "
      + "datum either change has ever had. "
      + "Nine closed fills settles none of this; what is settled is that the three "
      + "signals disagree, why the cash one disagrees, and that the estimator question "
      + "turns on a dispersion nobody can yet measure. The worst-fold walk-forward is "
      + "still the thing that would decide it.",
